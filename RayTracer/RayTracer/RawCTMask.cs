...
public class RawCtMask: Geometry
{
    ...
    {
        var diagonal = new Vector(_resolution[0] * _thickness[0] * _scale, _resolution[1] * _thickness[1] * _scale,
            _resolution[2] * _thickness[2] * _scale);
        var boundingEllipsoid = new Ellipsoid(_position + diagonal / 2, diagonal / 2, 1, Color.NONE);
        var (t1, t2) = boundingEllipsoid.GetFirstAndLastIntersection(line);
        if (t1 == 0 || t2 == 0)
        {
            return new Intersection();
        }

        var start = Math.Max(t1, minDist);
        var end = Math.Min(t2, maxDist);
        var stepSize = _scale;
        var firstIntersection = 0d;
        var normal = new Vector();
        var objectColor = new Color();
        var lastAlpha = 1d;
        var firstIntersectionVisible = false;
        for (var t = start; t <= end; t += stepSize)
        {
            var point = line.CoordinateToPosition(t);
            var partialColor = GetColor(point);
            if (partialColor.Alpha == 0) continue;
            if (!firstIntersectionVisible)
            {
                firstIntersection = t;
                normal = GetNormal(point);
                firstIntersectionVisible = true;
            }

            objectColor += partialColor * partialColor.Alpha * lastAlpha;
            lastAlpha *= 1 - partialColor.Alpha;
            if (lastAlpha < 0.01) break;
        }

        return new Intersection(true, firstIntersectionVisible, this, line, firstIntersection, normal, Material.FromColor(objectColor),
            objectColor);
    }
